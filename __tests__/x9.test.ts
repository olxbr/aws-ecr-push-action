...
describe("minimalSeverity", () => {
    test.each([
        ...
        const trivySeverity = x9.minimalSeverity(config)
...
describe('scanImage', () => {
    it('can scan', async () => {
        const execSpy: jest.SpyInstance = jest
            .spyOn(exec, 'getExecOutput')
            .mockImplementation(async (): Promise<exec.ExecOutput> => {
                return {
                    exitCode: 0,
                    stdout: '',
                    stderr: ''
        ...
        const results = await x9.scanImage('image', 'UNKNOWN')
...
`,
        ...
        expect(execSpy).toHaveBeenCalledWith(
            ...
            [
                ...
                "--build-arg", "TRIVY_SEVERITY=UNKNOWN",
                "."
            ],
            {
        ...
        expect(execSpy).toHaveBeenCalledWith(
            'docker',
            ['create', '--name', 'suspectcontainer', 'suspectimage'],
            {
                ignoreReturnCode: true,
                silent: true,
            }
        ...
        expect(execSpy).toHaveBeenCalledWith(
            'docker',
            ['cp', 'suspectcontainer:/scans', `./scans`],
            {
                ignoreReturnCode: true,
                silent: true,
            }
