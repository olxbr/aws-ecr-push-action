...
const reportImageThreats = (config) => {
  ...
  info(
    `report image threats cp ${config.x9ContainersDistro}.X9.Dockerfile done`
  );
  ...
  info(
    `report image threats reports got. Removing ${suspectContainerName} and ${suspectImageName}`
  );
  ...
  if (fs.existsSync(clamScanFile)) {
    ...
    if (totalsClam.some(isNaN)) {
      throw new Error(
        `report image threats file ${clamScanFileName} missing totals`
      );
    }
    if (totalsClam[0] > VIRUS_THRESHOLD) {
      throw new Error(
        `report image threats file ${clamScanFileName} threat threshold exceeded`
      );
    }
  }
