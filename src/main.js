...
const { executeSyncCmd, sortByKey } = require("./utils");
...
const defineRepositoryPolicy = async (config) => {
  ...
  const policy = buildPolicy({
    awsPrincipalRules: config.aws["AWS_PRINCIPAL_RULES"],
  });
...
const parseAuthToken = async (config) => {
  ...
  const response = await getAuthorizationToken({
    registryIds: [config.aws["AWS_ACCOUNT_ID"]],
  });
...
const pushImage = (config) => {
  ...
  return executeSyncCmd("docker", [
    "push",
    `${config.aws["ECR_ENDPOINT"]}/${config.repositoryNames[0]}:${config.tag}`,
  ]);
};
...
const deleteImages = async (config) => {
  ...
  info(
    `Searching images to delete from ${repositoryName}... Will be kept ${keepImages} images`
  );

  let imagesList = await describeImages({ repositoryName, maxResults, filter }); // NOSONAR
  info(
    `Found total of ${imagesList["imageDetails"].length} images in the repo...`
  );
  info(`Listing all images: ${JSON.stringify(imagesList)}`);

  info(`Finding ALL UNTAGGED images first...`);
  if (imagesList?.imageDetails) {
    let untaggedImgIds = [];
    ...
    if (untaggedImgIds.length > 0) {
      info(
        `Deleting ${
          untaggedImgIds.length
        } UNTAGGED images and will be cleaned ${untaggedCleanedSizeInMB.toFixed(
          2
        )} Megabytes...`
      );
      let delUntaggedImages = await batchDeleteImage({
        repositoryName: repositoryName,
        imageIds: untaggedImgIds,
      }); // NOSONAR
      ...
      imagesList["imageDetails"] = imagesList["imageDetails"].filter(
        (untagged) => !untaggedImgInfos.includes(untagged)
      );
      info(
        `Listing ONLY TAGGED images now ${
          imagesList["imageDetails"].length
        }: ${JSON.stringify(imagesList)}`
      );
    }
  }

  const sortedImageList = sortByKey(
    imagesList["imageDetails"],
    "imagePushedAt"
  );
  ...
  if (imagesToDelete.length > 0) {
    info(
      `Will be deleted ${imagesToDelete.length} images and will be cleaned ${(
        imagesSize /
        1024 /
        1024
      ).toFixed(2)} Megabytes`
